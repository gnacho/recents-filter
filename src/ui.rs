...
/// Builds the main window for the given application.
///
/// Returns the window so callers (the binary, or a screenshot tool) can use it.
/// When `interactive` is false, the folder picker (which needs a portal) is not
/// constructed, so the UI can be rendered offscreen.
pub fn build_window(app: &libadwaita::Application) -> libadwaita::ApplicationWindow {
    build_window_inner(app, true)
}

pub fn build_window_capture(app: &libadwaita::Application) -> libadwaita::ApplicationWindow {
    build_window_inner(app, false)
}
